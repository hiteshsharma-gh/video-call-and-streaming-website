...
import { Transport, Consumer, Device, IceParameters, IceCandidate, DtlsParameters, RtpParameters, MediaKind, RtpCapabilities } from 'mediasoup-client/types'
...
export function Event(roomId: string) {
  ...
  useEffect(() => {
    SignalingServer.init('ws://localhost:8000')

    SignalingServer.onMessage((msg) => {
      if (msg.event === INCOMING_EVENT_NAMES.CONNECTION_SUCCESS) {
    ...
    SignalingServer.onMessage((msg) => {
      if (msg.event === INCOMING_EVENT_NAMES.ROUTER_RTP_CAPABILITIES) {
        setRtpCapabilities(msg.data.rtpCapabilities as RtpCapabilities)
      }
    })
